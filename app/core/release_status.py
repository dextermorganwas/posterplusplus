...
def _compute_movie_status(theatrical_date, digital_date, physical_date, tmdb_status):
    today=date.today()
    ...
    if theatrical_date and theatrical_date <= today:
        if settings.cinema_max_age_years > 0 and (today-theatrical_date).days > settings.cinema_max_age_years*365:
            return "Streaming"
        return "Cinema"
    return "Streaming" if tmdb_status == "Released" else "Production"
...
async def fetch_movie_release_info(client, tmdb_id, tmdb_key, tmdb_status):
    ...
    if cached:
        cached["status"]=_compute_movie_status(_parse(cached.get("theatrical_date")),_parse(cached.get("digital_date")),_parse(cached.get("physical_date")),tmdb_status)
        return cached
    info={"status":None,"theatrical_date":None,"digital_date":None,"physical_date":None}
    ...
    earliest_theatrical=earliest_digital=latest_digital=earliest_physical=None
    for entry in payload.get("results",[]):
        for rd in entry.get("release_dates",[]):
            ...
            elif typ==3: earliest_theatrical=d if earliest_theatrical is None or d<earliest_theatrical else earliest_theatrical
    info={"status":_compute_movie_status(earliest_theatrical,earliest_digital,earliest_physical,tmdb_status),
